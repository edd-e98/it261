...
<body>
    <!-- This htmlspecialchars is to prevent a certain type of hacking called cross site scripting -->
    ...
    // then, we will ask ourselves: are any fields empty?
    // IF the fields are not empty, we will set them
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($_POST['bank'] == NULL) {
            echo ('<p class="error"> Please choose your institution! </p>');
        }
        ...
        $_POST['currency'], )) {
            ...
            if(!empty($name && $email && $amount && $currency && $bank)){
                ...
                echo ('
                    <div class="box">
                        <h2>Hello, '.$name.'.</h2>
                        <p>You now have <b>$'.number_format($dollars, 2).' American dollars</b>, and we will be e-mailing you at <b>'.$email.'</b> with your information, as well as depositing your funds at <b>'.$bank.' bank</b>.</p>
                    </div>
            ');
            }
        }
